  ...
  const initializeApp = async () => {
    try {
      ...
      if (tgUser) {
        ...
        await initializeUserInDatabase(tgUser, startParam)
      } else {
        // Демо пользователь для тестирования вне Telegram
        console.log("[v0] No Telegram user, creating demo user")
        const demoUser = {
          id: Date.now(),
      ...
      telegramWebApp.showAlert("Ошибка инициализации приложения")
    } finally {
      setLoading(false)
    }
  }

  const initializeUserInDatabase = async (tgUser: TelegramUser, startParam?: string) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setBalance(0.05)
        telegramWebApp.showAlert("Ошибка подключения к серверу. Используется демо режим.")
      }
    } catch (error) {
      console.error("[v0] Backend communication error:", error)
      // Fallback to demo balance
      setBalance(0.05)
      telegramWebApp.showAlert("Ошибка сети. Используется демо режим.")
    }
  }
        ...
        <div className="relative z-10 px-4 pb-24">
          {activeTab === "cases" && user && (
            <CasesSection
              ...
              onInventoryUpdate={(newItem: any) => setInventory((prev: any[]) => [...prev, newItem])}
            />
          )}
